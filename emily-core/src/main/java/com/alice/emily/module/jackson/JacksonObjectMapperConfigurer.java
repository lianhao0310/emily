...
public class JacksonObjectMapperConfigurer implements Configurer<ObjectMapper> {
    ...
    public JacksonObjectMapperConfigurer(JacksonProperties properties, List<Module> modules) {
    ...
    private void configurePropertyNamingStrategy(ObjectMapper objectMapper) {
        ...
        if (strategy != null) {
            ...
            // try replace default PropertyNamingStrategy with enhanced AnnotationSensitivePropertyNamingStrategy
            if (objectMapper.getPropertyNamingStrategy().getClass().isAssignableFrom(PropertyNamingStrategy.class)) {
