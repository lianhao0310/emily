    ...
    public boolean matches(Method method) {
        if (USER_DECLARED_METHODS.matches(method) && method.isAnnotationPresent(clazz)) {
            if (checkModifiers(method, methodModifiers)) {
                ...
                LOG.SYSTEM.warn("Ignoring @{} on method {}.{} due to illegal modifiers: {}", clazz.getSimpleName(), method.getDeclaringClass().getCanonicalName(),
    ...
    public boolean matches(Field field) {
        if (field.isAnnotationPresent(clazz)) {
            if (checkModifiers(field, fieldModifiers)) {
                ...
                LOG.SYSTEM.warn("Ignoring @{} on field {}.{} due to illegal modifiers: {}", clazz.getSimpleName(), field.getDeclaringClass().getCanonicalName(),
